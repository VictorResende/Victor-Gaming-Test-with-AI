...
import { LEVELS_CONFIG, levelBlurb, levelTitle } from '../config/levelsConfig';
import { SaveManager } from '../managers/SaveManager';
import { SafeArea, SafeAreaBounds, SafeAreaInsets } from '../utils/SafeArea';
import { t } from '../i18n/locales';
import { RELICS_CONFIG, relicDesc, relicName } from '../config/relicsConfig';
import {
  applyUiScene,
  bindControl,
  delayedStart,
...
export class LevelSelectScene extends Phaser.Scene {
  ...
  private endlessPick = false;

  constructor() {
    super('LevelSelectScene');
  }

  public init(data?: { endlessPick?: boolean }): void {
    this.endlessPick = !!data?.endlessPick;
  }
  ...
  public create(): void {
    ...
    addScreenTitle(this, width / 2, headerY, this.endlessPick ? t('endless') : t('level'));
  ...
  private openRelicsModal(): void {
    ...
    const heading = this.add.text(-panelW / 2 + 24, -panelH / 2 + 28, t('relicsTitle'), uiText(UI.text.primary, '20px', { fontStyle: '800' })).setOrigin(0, 0.5);
    const sub = this.add.text(0, -panelH / 2 + 56, t('relicsHint'), uiText(UI.text.muted, '12px')).setOrigin(0.5);
    ...
    relicEntries.forEach((relic, idx) => {
      ...
      const isEquipped = save.isRelicEquipped(relic.id);
      const isUnlocked = save.hasRelic(relic.id);
      ...
      const name = this.add.text(-cardW / 2 + 40, -12, relicName(relic), uiText(relic.colorHex || UI.text.primary, '12px', { fontStyle: '700' })).setOrigin(0, 0.5);
      const lockHint = !isUnlocked && relic.unlockLevelId
        ? t('relicLocked', { level: relic.unlockLevelId })
        : relicDesc(relic);
      const desc = this.add.text(-cardW / 2 + 40, 8, lockHint, uiText(UI.text.muted, '10px', { wordWrap: { width: cardW - 52 } })).setOrigin(0, 0.5);
      const status = this.add.text(cardW / 2 - 10, -cardH / 2 + 12, isEquipped ? t('relicEquipped') : (isUnlocked ? '' : '🔒'), uiText(isEquipped ? UI.text.success : UI.text.faint, '10px', { fontStyle: '700' })).setOrigin(1, 0.5);
      card.add([bg, icon, name, desc, status]);
      if (isUnlocked) {
        bindControl(card, cardW, cardH, () => {
          save.toggleRelic(relic.id);
          close();
