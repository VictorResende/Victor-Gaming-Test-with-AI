...
export class TechTreeScene extends Phaser.Scene {
  ...
  public create(): void {
    ...
    // 1. Fundo do Grimório Arcano Ancestral
    this.createGrimoireBackground(width, height);

    // 2. Cabeçalho Real Ancorado
    const headerY = this.safeInsets.top + 38;

    // Fita de Título do Grimório
    const headerRibbon = this.add.graphics();
    headerRibbon.fillStyle(0x2d180a, 0.96);
    headerRibbon.fillRoundedRect(width / 2 - 250, headerY - 22, 500, 44, 8);
    headerRibbon.lineStyle(2.5, 0xfacc15, 1);
    headerRibbon.strokeRoundedRect(width / 2 - 250, headerY - 22, 500, 44, 8);
    headerRibbon.lineStyle(1, 0x92400e, 0.8);
    headerRibbon.strokeRoundedRect(width / 2 - 246, headerY - 18, 492, 36, 6);

    this.add.text(width / 2, headerY, `📖 ${t('techTitle')}`, {
      fontSize: '22px',
      fontStyle: 'bold',
      color: '#fef08a',
      stroke: '#451a03',
    ...
    // Botão Voltar (Top Left Seguro, Hitbox 110x48px com Spring)
    ...
    backBtn.setSize(110, 48);
    backBtn.setInteractive(SafeArea.createTouchHitbox(110, 48), Phaser.Geom.Rectangle.Contains);
    ...
    // Contador de Estrelas / Cristais Arcanos (Top Right Seguro, Hitbox 120x48px)
    ...
    // 3. Grid 2x3 de Cartas de Feitiço em Pergaminho
    ...
    TECH_TREE_NODES.forEach((node, index) => {
      ...
      this.createParchmentSpellCard(x, y, cardW, cardH, node);
    });
  }

  // ==========================================
  // FUNDO DO GRIMÓRIO DE VELUDO & RUNAS
  // ==========================================
  private createGrimoireBackground(width: number, height: number): void {
    const bg = this.add.graphics();
    // Gradiente de Couro e Noite Arcana
    bg.fillGradientStyle(0x130d1d, 0x130d1d, 0x221332, 0x0d0714, 1);
    bg.fillRect(0, 0, width, height);

    // Moldura do Grimório (Borda dupla de ouro entalhado)
    bg.lineStyle(4, 0x78350f, 0.6);
    bg.strokeRect(12, 12, width - 24, height - 24);
    bg.lineStyle(1.5, 0xd97706, 0.4);
    bg.strokeRect(16, 16, width - 32, height - 32);

    // Runas Místicas e Partículas Mágicas Flutuantes
    const runes = ['᚛', '᚜', '⚡', '✨', '✦', '✧', 'ᚩ', 'ᚱ'];
    for (let i = 0; i < 40; i++) {
  ...
  // ==========================================
  // CARD DE FEITIÇO EM PERGAMINHO ENCANTADO
  // ==========================================
  private createParchmentSpellCard(x: number, y: number, w: number, h: number, node: TechNode): void {
    const save = SaveManager.getInstance();
    ...
    const bg = this.add.graphics();
    // Fundo em Pergaminho Envelhecido
    ...
    // Botão / Selo de Status (Hitbox expandida)
    ...
    if (!isUnlocked && canAfford) {
      ...
      attachSpringFeedback(container, this, {
        rippleColor: 0xfacc15,
        onClick: () => {
          const success = save.unlockTech(node.id, node.starCost);
          if (success) {
