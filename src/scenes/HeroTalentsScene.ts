...
export class HeroTalentsScene extends Phaser.Scene {
  ...
  public create(): void {
    ...
    addGhostButton(this, this.safeBounds.left + 186, headerY, t('towerTechTab'), () => {
      delayedStart(this, 'TechTreeScene');
    }, 132, 40);
    addScreenTitle(this, width / 2 + 24, headerY, t('heroTalentsTitle'));
  ...
  private createPerkCard(x: number, y: number, w: number, h: number, node: HeroPerkNode, highContrast: boolean): void {
    const save = SaveManager.getInstance();
    ...
    if (!isUnlocked && canAfford) {
      ...
      bindControl(container, w, h, () => {
        if (save.unlockHeroPerk(node.id, node.starCost)) {
